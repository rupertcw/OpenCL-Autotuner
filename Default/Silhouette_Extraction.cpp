...
SilhouetteExtraction::SilhouetteExtraction(std::string input, int num_training_frames, 
					   bool isFile)//: 
...
std::string SilhouetteExtraction::getInputName() {
  if(!hasInputSource) {
...
void SilhouetteExtraction::initialize(std::string clFileName) {
  if(!hasInputSource) {
    if(debug >= 0) {
  ...
  try {
    ...
    cl_context_properties cl_properties[] = 
      { CL_CONTEXT_PLATFORM, (cl_context_properties)(cl_platforms[0])(), 0};
    
    cl_context = cl::Context(CL_DEVICE_TYPE_GPU, cl_properties);
        
    cl_devices = cl_context.getInfo<CL_CONTEXT_DEVICES>();

    cl_sources = cl::Program::Sources(1,std::make_pair(clFile,kernels_size));
 ...
 * - Don't convert, use Y from YUV. Convert YUV to HSV if possible, to get H.
 */
void SilhouetteExtraction::performTrainingStep() {
  ...
  try {
	...
	exit(1);
      ...
      /* STATS:
	 We will calculate each iteration of the training step, even if one of the kernels
	 are used later in the algorithm */
      // start_time = getTime();

      /* Allocate buffer for in-data (RGB frame data). 
	 Could maybe use OpenCL 2D Image here? Yup. TODO */
      cl_rgb_frame = cl::Buffer(cl_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
				(3*size)*currentFrame.elemSize1(), 
				(uchar*) currentFrame.data);        
      /* Set args for kernel */
      ...
      cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_convert_rgb_img,
      				       cl::NullRange, cl::NDRange(width,height),
      				       cl::NDRange(16,16), NULL, &cl_event);	

      /* 32x32 */
      // cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_convert_rgb_img,
      // 				       cl::NullRange, cl::NDRange(width,height),
      // 				       cl::NDRange(32,32), NULL, &cl_event);	
	 ...
	 pixel (1 = Laplace, 2 = Gaussian) */
      ...
      /* STATS */
      // end_time = getTime();
      // diff_time = ts_sub(end_time,start_time);
      // trainingstep_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);
    }
...
bool SilhouetteExtraction::extractFromNextFrame() {
  cv::Mat tmpFrame = nextFrame();
  if(tmpFrame.data == NULL)
    return false;
  ...
  // struct timespec end_time;
  // struct timespec end_time_total;

  // struct timespec diff_time;
  // struct timespec diff_time_total;
  ...
  // end_time = getTime();
  // diff_time = ts_sub(end_time,start_time);
  // convert_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);
  

  // Update the background model from new frame data
  // start_time = getTime();
  updateBgModelForFrame();
  // end_time = getTime();
  // diff_time = ts_sub(end_time,start_time);
  // update_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);

  // Extract silhouettes. TODO. Might be split into multiple functions.
  
  // start_time = getTime();
  // uint64_t start64 = (uint64_t)start_time.tv_sec * BILLION + start_time.tv_nsec;
  subtractBackgroundAndThreshold();
  // end_time = getTime();
  // diff_time = ts_sub(end_time,start_time);

  // uint64_t end64 = (uint64_t)end_time.tv_sec * BILLION + start_time.tv_nsec;
  // bgsub_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);

  // start_time = getTime();
  closeAndOpenImage(); // two kernels, image2d
  ...
  // end_time = getTime();
  // diff_time = ts_sub(end_time,start_time);
  // closeopen_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);
  ...
  // end_time = getTime();
  // diff_time = ts_sub(end_time,start_time);
  // ccl_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);

  // start_time = getTime();
  updateBG();
  // end_time = getTime();
  // diff_time = ts_sub(end_time,start_time);
  // update_bg_stats.insert(diff_time.tv_sec * BILLION + diff_time.tv_nsec);

  // end_time_total = getTime();
  // diff_time_total = ts_sub(end_time_total,start_time_total);
  // total_stats.insert(diff_time_total.tv_sec * BILLION + diff_time_total.tv_nsec);

  return true;
}

void SilhouetteExtraction::convertNextFrame() {
  cl::Event cl_event;
...
void SilhouetteExtraction::updateBgModelForFrame() {
  cl::Event cl_event;
...
void SilhouetteExtraction::subtractBackgroundAndThreshold() {
  cl::Event cl_event;
...
void SilhouetteExtraction::closeAndOpenImage() {
  cl::Event cl_event;
  try {
...
/* Call the dilate/erode kernels for closing/opening the image */
void SilhouetteExtraction::closeAndOpenImage2() {
  cl::Event cl_event;
  try {
    /* Start closing/opening */    
    int local_xy = 16;
...
void SilhouetteExtraction::closeAndOpenImage3() {
  cl::Event cl_event;
  try {
    /* Start closing/opening */    
...
void SilhouetteExtraction::labelConnectedComponents() {
  cl::Event cl_event;

  int scanning_iterations = 0;
  image2d_fgmask2 = cl::Image2D(cl_context, CL_MEM_READ_WRITE,
  ...
  try {
    ...
    image2d_fgmask = cl::Image2D(cl_context, CL_MEM_READ_WRITE,
				 uint32_format, width, height, 0, NULL);
 ...
 * could be done.
 */
void SilhouetteExtraction::labelConnectedComponents2() {
  cl::Event cl_event;
  ...
  try {
    ...
    while(tileSize < width || tileSize < height) {
      ...
      cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_ccl2_2,
    				       cl::NullRange, 
    				       global_range,
    				       local_range,
    				       NULL,&cl_event);
    ...
    //   /* END TEST! */
      ...
      if(tileSize < width || tileSize < height) {
      	...
      	cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_ccl2_3,
      					 cl::NullRange, 
      					 grid,
      					 block,
      					 NULL,&cl_event);
...
void SilhouetteExtraction::updateBG() {
  cl::Event cl_event;
...
void SilhouetteExtraction::setupBuffersForExtraction() {
  ...
  try {
    ...
    image2d_fgmask = cl::Image2D(cl_context, CL_MEM_READ_WRITE,
				 bin_img_format, width, height, 0, NULL);
    image2d_fgmask2 = cl::Image2D(cl_context, CL_MEM_READ_WRITE,
				  bin_img_format, width, height, 0, NULL);
...
void SilhouetteExtraction::showOutput() {
  ...
  IplImage *testImg = new IplImage(testImage);
  cvShowImage("Test, lData[curImg]",testImg);
  IplImage *origFrame = new IplImage(currentFrame);
  cvShowImage("Original frame",origFrame);
}
