...
SilhouetteExtraction::SilhouetteExtraction(string input, int num_training_frames, bool isFile, int typ, bool tiles, int device)
...
//        cout << "WG Size6: " << maxWorkgroupSize << endl;
        ...
        if(cl_err.err() == CL_BUILD_PROGRAM_FAILURE) {
            ...
            cl_program.getBuildInfo(cl_devices[0], CL_PROGRAM_BUILD_LOG, &buildLog);
            cout << buildLog << endl;
        }
        exit(1);
    }
...
void SilhouetteExtraction::initialize(string clFileName, int workgroupSize) {
    ...
    if(inputIsFile) {
        ...
        outTest = (uchar*) calloc(size, sizeof(uchar));
        outTest2 = (uchar*) calloc(size, sizeof(uchar));
...
bool SilhouetteExtraction::performTrainingStep(int workgroupSize)
{
    cl::Event cl_event;
    int local_x, local_y;
    double time, bestTime1 = 99999, bestTime2 = 99999;
    ...
    try {
        int tsx = 1, tsy = 1, besttsx1, besttsy1, besttsx2, besttsy2;
        ...
        for (int i=0;i<N;i++) {
            ...
            if (tiling) {
                cl_kernel_convert_rgb_img.setArg(3, 1);
                cl_kernel_mean_variance_kurtosis.setArg(10, 1);
                
                for (tsx=1; tsx<width; tsx++) {
                    for (tsy=1; tsy<height; tsy++) {
                        if ((width%tsx == 0) && (height%tsy == 0)) {
                            /// CONVERT RGB
                            cl_kernel_convert_rgb_img.setArg(4, tsx);
                            cl_kernel_convert_rgb_img.setArg(5, tsy);
                            ...
                            time = end-start;
//                            cout << "time for tile convertRGB kernel: " << time << endl;
                            ...
                            /// CALCULATE KURTOSIS
                            cl_kernel_mean_variance_kurtosis.setArg(11, tsx);
                            cl_kernel_mean_variance_kurtosis.setArg(12, tsy);
                            ...
                            time = end-start;
//                            cout << "time for tile calcKurtosis kernel: " << time << endl;
                ...
                cout << "Best Convert RGB - Runtime: " << bestTime1 << ", TSX: " << besttsx1 << ", TSY: " << besttsy1 << endl;
                cout << "Best Calculate Kurtosis - Runtime: " << bestTime2 << ", TSX: " << besttsx2 << ", TSY: " << besttsy2 << endl;
            } else {
                ...
                cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_convert_rgb_img,
                                                ...
                                                cl::NDRange(local_x, local_y), NULL, &cl_event); // change local based on new global
                cl_event.wait();
                ...
                /// CALCULATE KURTOSIS
                cl_kernel_mean_variance_kurtosis.setArg(10, 0);
                cl_kernel_mean_variance_kurtosis.setArg(11, tsx);
                cl_kernel_mean_variance_kurtosis.setArg(12, tsy);
    ...
    //            int test[size*2];
    //
    //            cl_cmdqueue.enqueueReadBuffer(cl_Data, CL_TRUE, 0, size*sizeof(int), test, NULL, NULL);
    //
    //            for (int i=0; i<6; i+=2) {
    //                cout << "Y: " << test[i] << endl;
    //                cout << "H: " << test[i+1] << endl;
    //            }
            }
        }
    }
...
void SilhouetteExtraction::convertNextFrame(int workgroupSize)
{
    ...
    try {
        int tsx = 1, tsy = 1, besttsx, besttsy;
        cl_rgb_frame = cl::Buffer(cl_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, (3*size)*currentFrame.elemSize1(), (uchar*) currentFrame.data);
        ...
        if (tiling) {
            ...
            cout << "Best Convert RGB - Runtime: " << bestTime << ", TSX: " << besttsx << ", TSY: " << besttsy << endl;
        } else {
            cl_kernel_convert_rgb_img.setArg(3, 0);
            cl_kernel_convert_rgb_img.setArg(4, tsx);
            cl_kernel_convert_rgb_img.setArg(5, tsy);
...
void SilhouetteExtraction::updateBgModelForFrame(int workgroupSize)
{
    ...
    try {
        int tsx = 1, tsy = 1, besttsx, besttsy;
        if (tiling) {
            cl_kernel_update_bg_model.setArg(4, 1);
            
            for (tsx=1; tsx<width; tsx++) {
                for (tsy=1; tsy<height; tsy++) {
                    if ((width%tsx == 0) && (height%tsy == 0)) {
                        cl_kernel_update_bg_model.setArg(5, tsx);
                        cl_kernel_update_bg_model.setArg(6, tsy);
            ...
            cout << "Best UpdateBgModel - Runtime: " << bestTime << ", TSX: " << besttsx << ", TSY: " << besttsy << endl;
        } else {

            cl_kernel_update_bg_model.setArg(4, 0);
            cl_kernel_update_bg_model.setArg(5, tsx);
            cl_kernel_update_bg_model.setArg(6, tsy);
...
void SilhouetteExtraction::subtractBackgroundAndThreshold(int workgroupSize)
{
    ...
    try {
        ...
        if (tiling) {
            cl_kernel_subtract_bg_and_threshold.setArg(7, 1);
            
            for (tsx=1; tsx<width; tsx++) {
                for (tsy=1; tsy<height; tsy++) {
                    if ((width%tsx == 0) && (height%tsy == 0)) {
                        cl_kernel_subtract_bg_and_threshold.setArg(8, tsx);
                        cl_kernel_subtract_bg_and_threshold.setArg(9, tsy);
            ...
            cout << "Best subAndThresh - Runtime: " << bestTime << ", TSX: " << besttsx << ", TSY: " << besttsy << endl;
        } else {
            cl_kernel_subtract_bg_and_threshold.setArg(7, 0);
            cl_kernel_subtract_bg_and_threshold.setArg(8, tsx);
            cl_kernel_subtract_bg_and_threshold.setArg(9, tsy);
...
void SilhouetteExtraction::closeAndOpenImage(int workgroupSize)
{
    cl::Event cl_event;
    int local_x, local_y;
    double time1, time2, bestTime1 = 99999, bestTime2 = 99999;
    ...
    try {
        ...
        int tsx = 1, tsy = 1, besttsx1, besttsy1, besttsx2, besttsy2;
        image2d_fgmask1 = image2d_fgmask;
        image2d_fgmask2 = image2d_fgmask3;
        ...
        if (tiling) {
            for (tsx=1; tsx<width; tsx++) {
                for (tsy=1; tsy<height; tsy++) {
                    if ((width%tsx == 0) && (height%tsy == 0)) {
                        ...
                        cl_kernel_dilate.setArg(2, 1);
                        cl_kernel_dilate.setArg(3, tsx);
                        cl_kernel_dilate.setArg(4, tsy);
                        ...
                        cl_kernel_erode.setArg(2, 1);
                        cl_kernel_erode.setArg(3, tsx);
                        cl_kernel_erode.setArg(4, tsy);
                        ...
                        // save result
                        image2d_fgmask4 = image2d_fgmask;
                        image2d_fgmask5 = image2d_fgmask2;
                        
                        // copy original data back in
                        image2d_fgmask = image2d_fgmask1;
                        image2d_fgmask2 = image2d_fgmask3;
                    }
                }
            }
            cout << "Best Dilate - Runtime: " << bestTime1 << ", TSX: " << besttsx1 << ", TSY: " << besttsy1 << endl;
            cout << "Best Erode - Runtime: " << bestTime2 << ", TSX: " << besttsx2 << ", TSY: " << besttsy2 << endl;
            
            // Copy result back
            image2d_fgmask = image2d_fgmask4;
            image2d_fgmask2 = image2d_fgmask5;
        } else {
            // 1. Dilate
            cl_kernel_dilate.setArg(0, image2d_fgmask);
            cl_kernel_dilate.setArg(1, image2d_fgmask2);
            cl_kernel_dilate.setArg(2, 0);
            cl_kernel_dilate.setArg(3, tsx);
            cl_kernel_dilate.setArg(4, tsy);
            ...
            cout << "Dilate: " << time1 << endl;
            cout << "Erode: " << time2 << endl;
...
void SilhouetteExtraction::labelConnectedComponents(int workgroupSize)
{
    cl::Event cl_event;
    int local_x, local_y;
    double time, bestTime1 = 99999, bestTime2 = 99999, bestTime3 = 99999;
    ...
    try {
        int tsx = 1, tsy = 1, besttsx1, besttsy1, besttsx2, besttsy2, besttsx3, besttsy3;
        image2d_fgmask1 = image2d_fgmask;
        image2d_fgmask3 = image2d_fgmask2;
            ...
            for (tsx=1; tsx<width; tsx++) {
                for (tsy=1; tsy<height; tsy++) {
                    if ((width%tsx == 0) && (height%tsy == 0)) {
                        ...
                        cl_kernel_ccl_init_labels.setArg(2, 1);
                        cl_kernel_ccl_init_labels.setArg(3, tsx);
                        cl_kernel_ccl_init_labels.setArg(4, tsy);
                        ...
                        while(1) {
                            ...
                            cl_kernel_ccl_scanning.setArg(4, 1);
                            cl_kernel_ccl_scanning.setArg(5, tsx);
                            cl_kernel_ccl_scanning.setArg(6, tsy);
...
//                            cout << "here2" << endl;
                        ...
                        cl_kernel_ccl_analysis.setArg(2, 1);
                        cl_kernel_ccl_analysis.setArg(3, tsx);
                        cl_kernel_ccl_analysis.setArg(4, tsy);
            ...
            cout << "Best CCL_Init - Runtime: " << bestTime1 << ", TSX: " << besttsx1 << ", TSY: " << besttsy1 << endl;
            cout << "Best CCL_Scan - Runtime: " << bestTime2 << ", TSX: " << besttsx2 << ", TSY: " << besttsy2 << endl;
            cout << "Best CCL_Anal - Runtime: " << bestTime3 << ", TSX: " << besttsx3 << ", TSY: " << besttsy3 << endl;
            ...
            cl_kernel_ccl_init_labels.setArg(0, image2d_fgmask);
            cl_kernel_ccl_init_labels.setArg(1, image2d_fgmask2);
            cl_kernel_ccl_init_labels.setArg(2, 0);
            cl_kernel_ccl_init_labels.setArg(3, tsx);
            cl_kernel_ccl_init_labels.setArg(4, tsy);
            ...
            while(1) {
                ...
                cl_kernel_ccl_scanning.setArg(2, cl_uint_val);
                cl_kernel_ccl_scanning.setArg(3, cl_uint_image);
                cl_kernel_ccl_scanning.setArg(4, 0);
                cl_kernel_ccl_scanning.setArg(5, tsx);
                cl_kernel_ccl_scanning.setArg(6, tsy);
            ...
            cl_kernel_ccl_analysis.setArg(1, cl_uint_image);
            cl_kernel_ccl_analysis.setArg(2, 0);
            cl_kernel_ccl_analysis.setArg(3, tsx);
            cl_kernel_ccl_analysis.setArg(4, tsy);
...
void SilhouetteExtraction::updateBG(int workgroupSize)
{
    ...
    try {
        ...
        if (tiling) {
            cl_kernel_update_bg.setArg(3, 1);
            
            for (tsx=1; tsx<width; tsx++) {
                for (tsy=1; tsy<height; tsy++) {
                    if ((width%tsx == 0) && (height%tsy == 0)) {
                        cl_kernel_update_bg.setArg(4, tsx);
                        cl_kernel_update_bg.setArg(5, tsy);
            ...
            cout << "Best updateBG - Runtime: " << bestTime << ", TSX: " << besttsx << ", TSY: " << besttsy << endl;
        } else {
            cl_kernel_update_bg.setArg(3, 0);
            cl_kernel_update_bg.setArg(4, tsx);
            cl_kernel_update_bg.setArg(5, tsy);
...
//        cl_kernel_ccl2_4 = cl::Kernel(cl_program, "CCL_kernel_4");
        ...
        image2d_fgmask1 = cl::Image2D(cl_context, CL_MEM_READ_WRITE, bin_img_format, width, height, 0, NULL);
        image2d_fgmask2 = cl::Image2D(cl_context, CL_MEM_READ_WRITE, bin_img_format, width, height, 0, NULL);
        image2d_fgmask3 = cl::Image2D(cl_context, CL_MEM_READ_WRITE, bin_img_format, width, height, 0, NULL);
        image2d_fgmask4 = cl::Image2D(cl_context, CL_MEM_READ_WRITE, bin_img_format, width, height, 0, NULL);
        image2d_fgmask5 = cl::Image2D(cl_context, CL_MEM_READ_WRITE, bin_img_format, width, height, 0, NULL);
        ...
        // image2d_ccl = cl::Image2D(cl_context, CL_MEM_READ_WRITE, uint32_format, width, height, 0, NULL);
        // image2d_ccl2 = cl::Image2D(cl_context, CL_MEM_READ_WRITE, uint32_format, width, height, 0, NULL);
