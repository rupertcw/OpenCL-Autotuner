...
__kernel void convert_rgb_to_h_and_y(global uchar* rgbimg, global int2* Data, int width, int tiles, int tsx, int tsy)
{
    if (tiles) {
        int col = get_global_id(0) * tsx;
        int row = get_global_id(1) * tsy;
        
        for (int py = row; py < row+tsy; py++) {
            for (int px = col; px < col+tsx; px++) {
                int id = (py*width) + px;
...
__kernel void calc_mean_variance_kurtosis(int N_max, int size, int n, global int2* data,
                                          ...
                                          global int2* DistMap, int width, int tiles, int tsx, int tsy)
{
    if (tiles) {
        int col = get_global_id(0) * tsx;
        int row = get_global_id(1) * tsy;
...
__kernel void update_bg_model(global int2* Data, global float2* lc_Model, global float4* M_V, global int2* DistMap, int tiles, int tsx, int tsy)
{
    if (tiles) {
        int col = get_global_id(0) * tsx;
        int row = get_global_id(1) * tsy;
...
__kernel void subtract_and_threshold(global int2* Data, global float2* lcModel, global float4* M_V,
                                     ...
                                     global uchar* output_buffer, int tiles, int tsx, int tsy)
{
    if (tiles) {
        int col = get_global_id(0) * tsx;
        int row = get_global_id(1) * tsy;
...
__kernel void background_update(__read_only image2d_t input_img, global int2* Data, global float4* M_V, int tiles, int tsx, int tsy)
{
    if (tiles) {
        int col = get_global_id(0) * tsx;
        int row = get_global_id(1) * tsy;
...
__kernel void dilate_image(__read_only  image2d_t input_img, __write_only image2d_t output_img, int tiles, int tsx, int tsy)
{
    /* Sampler for image */
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
...
/* Opposite of dilate. */
__kernel void erode_image(__read_only image2d_t input_img, __write_only image2d_t output_img, int tiles, int tsx, int tsy)
{
    /* Sampler for image */
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
 ...
 */
__kernel void CCL_init_labels(__read_only image2d_t input_img, __write_only image2d_t output_img, int tiles, int tsx, int tsy)
{
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
...
__kernel void CCL_analysis(__read_only image2d_t input_image, global uint* output_image, int tiles, int tsx, int tsy)
{
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
