...
__kernel void convert_rgb_to_h_and_y(global uchar* rgbimg, global int2* Data, int width, int tiles, int ts)
{
    if (tiles) {
        int col = get_global_id(0) * ts;
        int row = get_global_id(1) * ts;
//        int col = get_global_id(0);
//        int row = get_global_id(1);
        
        for (int py = row; py < py+ts; py++) {
            for (int px = col; px < px+ts; px++) {
                int id = (py*width) + px;
//                int id = (row*80) + col;
...
__kernel void calc_mean_variance_kurtosis(int N_max, int size, int n, global int2* data,
                                          ...
                                          global int2* DistMap, int width, int tiles, int ts)
{
    if (tiles) {
        int col = get_global_id(0) * ts;
        int row = get_global_id(1) * ts;
...
__kernel void update_bg_model(global int2* Data, global float2* lc_Model,
                              global float4* M_V, global int2* DistMap, int tiles, int ts)
{
    if (tiles) {
        int col = get_global_id(0) * ts;
        int row = get_global_id(1) * ts;
        ...
        for (int py = row; py < py+ts; py++) {
            for (int px = col; px < px+ts; px++) {
                ...
                float lStdDev = native_sqrt(MV.s2);
                float cStdDev = native_sqrt(MV.s3);
                int lDist = dist_map.s0;
                int cDist = dist_map.s1;
                
                /* Calculate luma model */
                float div1 = native_divide(1, lStdDev);
                float div2 = native_divide((float)3, lDist);
                div2 = tgamma(div2);
                float div3 = native_divide((float)1, lDist);
                div3 = tgamma(div3);
                div3 = native_divide(div2, div3);
                
                float Yl = div1 * (native_powr(div3, 0.5f));
                
                div1 = native_divide((float)1, lDist);
                div1 = tgamma(div1);
                div1 = native_divide((lDist * Yl), (2*div1));
                div2 = native_powr(-Yl, lDist);
                div3 = native_powr((fabs((float)lc_data.s0 - MV.s0)), lDist);
                
                model.s0 = div1 * native_exp(div2 * div3);
                
                /* Calculate color model */
                div1 = native_divide(1, cStdDev);
                div2 = native_divide((float)3, cDist);
                div2 = tgamma(div2);
                div3 = native_divide((float)1, cDist);
                div3 = tgamma(div3);
                div3 = native_divide(div2, div3);
                
                float Yc = div1 * (native_powr(div3, 0.5f));
                
                div1 = native_divide((float)1, cDist);
                div1 = tgamma(div1);
                div1 = native_divide((cDist * Yc), (2*div1));
                div2 = native_powr(-Yl, cDist);
                div3 = native_powr((fabs((float)lc_data.s1 - MV.s1)), cDist);
                
                model.s1 = div1 * native_exp(div2 * div3);
...
__kernel void subtract_and_threshold(global int2* Data, global float2* lcModel, global float4* M_V,
                                     ...
                                     global uchar* output_buffer, int tiles, int ts)
{
    if (tiles) {
        int col = get_global_id(0) * ts;
        int row = get_global_id(1) * ts;
...
__kernel void background_update(__read_only image2d_t input_img, global int2* Data, global float4* M_V, int tiles, int ts)
{
    if (tiles) {
        int col = get_global_id(0) * ts;
        int row = get_global_id(1) * ts;
...
/* Dilation: This implementation is assuming a 3x3 structuring element. */
__kernel void dilate_image(__read_only  image2d_t input_img, __write_only image2d_t output_img, int tiles, int ts)
{
    /* Sampler for image */
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
...
/* Opposite of dilate. */
__kernel void erode_image(__read_only  image2d_t input_img, __write_only image2d_t output_img, int tiles, int ts)
{
    /* Sampler for image */
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
    ...
    if (tiles) {
        ...
        for (int py = row; py < row+ts; py++) {
            for (int px = col; px < col+ts; px++) {
                int x = px;
                int y = py;
 ...
 */
__kernel void CCL_init_labels(__read_only image2d_t input_img, __write_only image2d_t output_img, int tiles, int ts)
{
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
...
__kernel void CCL_scanning(__read_only image2d_t input_img, __write_only image2d_t output_img, global uint* isNotDone, global uint* img,
                           int tiles, int ts
...
__kernel void CCL_analysis(__read_only image2d_t input_image, global uint* output_image, int tiles, int ts)
{
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
