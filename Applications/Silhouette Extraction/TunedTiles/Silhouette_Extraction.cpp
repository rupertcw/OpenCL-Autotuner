        ...
        for (int i=0;i<N;i++) {
            ...
            if (tiling) {
                ...
                cl_kernel_mean_variance_kurtosis.setArg(10, 1);
                    ...
                    //                for (ts=1; ts<local_x; ts++) {
                    if ((width%ts == 0) && (height%ts == 0)) {
                        /// CONVERT RGB
                        cl_kernel_convert_rgb_img.setArg(4, ts);
                        double start = omp_get_wtime();
                        
                        cout << "before tiling" << endl;
                        cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_convert_rgb_img,
                                                         cl::NullRange, cl::NDRange(width/ts, height/ts),
                                                         cl::NDRange(4, 4), NULL, &cl_event);
                        cl_event.wait();
                        cout << "after tiling" << endl;
                        
                        double end = omp_get_wtime();
                        cout << "time for tile convertRGB kernel: " << end-start << endl;
                        
                        /// CALCULATE KURTOSIS
                        cl_kernel_mean_variance_kurtosis.setArg(11, ts);
                        ...
                        cout << "time for def calcKurtosis kernel: " << end-start << endl;
                    }
//                }
            } else {
...
//            for (ts=1; ts<local_x; ts++) {
                if ((width%ts == 0) && (height%ts == 0)) {
                    cl_kernel_update_bg_model.setArg(5, ts);
                    ...
                    double end = omp_get_wtime();
                    cout << "time for tile updateBG kernel: " << end-start << endl;
                }
//            }
        } else {
...
//            for (ts=1; ts<local_x; ts++) {
                if ((width%ts == 0) && (height%ts == 0)) {
                    cl_kernel_subtract_bg_and_threshold.setArg(8, ts);
                    ...
                    double end = omp_get_wtime();
                    cout << "time for tile subAndThresh kernel: " << end-start << endl;
                }
//            }
        } else {
...
//            for (ts=1; ts<local_x; ts++) {
                if ((width%ts == 0) && (height%ts == 0)) {
                    ...
                    double end = omp_get_wtime();
                    cout << "time for tile dilate kernel: " << end-start << endl;
                    ...
                    cout << "time for tile erode1 kernel: " << end-start << endl;
                    ...
                    cout << "time for tile erode2 kernel: " << end-start << endl;
                    ...
                    cout << "time for tile dilate2 kernel: " << end-start << endl;
...
//            }
            ...
            double end = omp_get_wtime();
            cout << "time for def dilate kernel: " << end-start << endl;
            ...
            cout << "time for def erode kernel: " << end-start << endl;
...
//            for (ts=1; ts<local_x; ts++) {
                if ((width%ts == 0) && (height%ts == 0)) {
                    cl_kernel_ccl_init_labels.setArg(3, ts);
                    ...
                    double end = omp_get_wtime();
                    cout << "time for tile CCL_Init kernel: " << end-start << endl;
                    ...
                    while(1) {
                        ...
                        cl_kernel_ccl_scanning.setArg(5, ts);
                        ...
                        cout << "time for tile CCL_Scan kernel: " << end-start << endl;
                    ...
                    cl_kernel_ccl_analysis.setArg(3, ts);
                    ...
                    cout << "time for tile CCL_Anal kernel: " << end-start << endl;
                }
//            }
        } else {
            cl_kernel_ccl_init_labels.setArg(0, image2d_fgmask);
            ...
            double end = omp_get_wtime();
            cout << "time for def CCL_Init kernel: " << end-start << endl;
            ...
            while(1) {
                ...
                cout << "time for def CCL_Scan kernel: " << end-start << endl;
            ...
            cout << "time for def CCL_Anal kernel: " << end-start << endl;
        }
...
//            for (ts=1; ts<local_x; ts++) {
                if ((width%ts == 0) && (height%ts == 0)) {
                    cl_kernel_update_bg.setArg(4, ts);
