...
//__kernel void convert_rgb_to_h_and_y(global uchar* rgbimg, global int2* Data, int width, int tiles, int tileSize) {
__kernel void soa_convert_rgb_to_h_and_y(global uchar* red, global uchar* green, global uchar* blue, global int* data1, global int* data2, int width, int tiles, int tileSize)
{
    int H=0, S, I;
 ...
 */
__kernel void soa_calc_mean_variance_kurtosis(int N_max, int size, int n,
                                          global int* data1, global int* data2,
                                          global float* M_V1, global float* M_V2, global float* M_V3, global float* M_V4,
                                          global float* M_21, global float* M_22,
                                          global float* M_31, global float* M_32,
                                          global float* M_41, global float* M_42,
                                          global int* distMap1, global int* distMap2,
                                          int width)
{
    ...
    /* data: l first, then c */
    int lcData1 = data1[id];
    int lcData2 = data2[id];
    ...
    if(n == 0) {
        MV1 = MV1 * first_step; // multiply all elements with 0.0 or 1.0
        MV2 = MV2 * first_step;
        MV3 = MV3 * first_step;
        MV4 = MV4 * first_step;
    ...
    // Update variance
    MV3 = MV3 + delta * (lcData1 - MV1);
    MV3 = MV3 * first_step;             // If first run, set to 0.0
    M41 = M41 + term1 * delta_n2 * (n*n - 3*n + 3) + 6 * delta_n2 * M21 - 4 * delta_n * M31;
    M31 = M31 + term1 * delta_n * (n - 2) - 3 * delta_n * M21;
    M21 = M21 + term1;
    ...
    //Update variance
    MV4 = MV4 + delta * (lcData2 - MV2);
    MV4 = MV4 * first_step;             // If first run, set to 0.0
    M42 = M42 + term1 * delta_n2 * (n*n - 3*n + 3) + 6 * delta_n2 * M21 - 4 * delta_n * M32;
    M32 = M32 + term1 * delta_n * (n - 2) - 3 * delta_n * M22;
    M22 = M22 + term1;
    ...
    if (n == N_max) {
        /* Calculate excess kurtosis:
         M4 is in .s3, M2 is in .s1
         (count*M4) / (M2*M2) - 3 */
        float lKurt = (n*M41)/(M21*M21) - 3;
        float cKurt = (n*M42)/(M22*M22) - 3;
        
        /* Set map for distribution to use
           Using step-function to avoid the if's:
           Will give 1+0 if 0.0 < lKurt (lKurt > 0.0)), 1+1 otherwise */
        distMap1[id] = (int) (step(lKurt, 0.0f) + 1);
        
        // Will give 1+0 if 0.0 < lKurt (lKurt > 0.0)), 1+1 otherwise
        distMap2[id] = (int) (step(cKurt, 0.0f) + 1);
    }
}

__kernel void calc_mean_variance_kurtosis(int N_max, int size, int n, global int2* data,
                                          global float4* M_V, global float2* M_2,
                                          global float2* M_3, global float2* M_4,
                                          global int2* DistMap, int width)
{
    int id = get_global_id(0);
...
//__kernel void calc_mean_variance_kurtosis(int N_max, int size, int n,
//                                          global int* data1, global int* data2,
//                                          global float* M_V1, global float* M_V2, global float* M_V3, global float* M_V4,
//                                          global float* M_21, global float* M_22,
//                                          global float* M_31, global float* M_32,
...
//    /* M_V contains: luma-mean, color-mean,
//     luma-variance, color-variance, in that order */
//    float4 MV = (M_V1[id], M_V2[id], M_V3[id], M_V4[id]);
//    
//    /* M2-4's have luma first, then color */
//    float2 M2 = (M_21[id], M_22[id]);
//
//    float2 M3 = (M_31[id], M_32[id]);
//
//    float2 M4 = (M_41[id], M_42[id]);
//    
//    
//    /* data: l first, then c */
//    int lcData1 = data1[id];
...
//    
//    // Update variance
//    MV.s2 = MV.s2 + delta * (lcData1 - MV.s0);
//    MV.s2 = MV.s2 * first_step; // If first run, set to 0.0
//    M4.s0 = M4.s0 + term1 * delta_n2 * (n*n - 3*n + 3) + 6 * delta_n2 * M2.s0 - 4 * delta_n * M3.s0;
//    M3.s0 = M3.s0 + term1 * delta_n * (n - 2) - 3 * delta_n * M2.s0;
//    M2.s0 = M2.s0 + term1;
...
//    
//    //Update variance
//    MV.s3 = MV.s3 + delta * (lcData2 - MV.s1);
//    MV.s3 = MV.s3 * first_step; // If first run, set to 0.0
//    M4.s1 = M4.s1 + term1 * delta_n2 * (n*n - 3*n + 3) + 6 * delta_n2 * M2.s0 - 4 * delta_n * M3.s1;
//    M3.s1 = M3.s1 + term1 * delta_n * (n - 2) - 3 * delta_n * M2.s1;
//    M2.s1 = M2.s1 + term1;
...
__kernel void soa_update_bg_model(global int* Data1, global int* Data2,
                                  global float* lc_Model1, global float* lc_Model2,
                                  global float* M_V1, global float* M_V2, global float* M_V3, global float* M_V4,
                                  global int* DistMap1, global int* DistMap2)
{
    /* id for this pixel */
    ...
    float model1 = lc_Model1[id];
    float model2 = lc_Model2[id];
    
    int dist_map1 = DistMap1[id];
    int dist_map2 = DistMap2[id];
    
    int lc_data1 = Data1[id];
    int lc_data2 = Data2[id];
    
    /* Standard deviation is sqrt of variance */
    float lStdDev = sqrt(MV3);
    float cStdDev = sqrt(MV4);
    int lDist = dist_map1;
    int cDist = dist_map2;
    ...
    model1 = ((lDist * Yl) / (2*tgamma((float)1/lDist))) * exp(pow(-Yl, lDist) * pow((fabs((float)lc_data1 - MV1)), lDist));
    
    /* Calculate color model */
    float Yc = (1/cStdDev) * (pow((tgamma((float)3/cDist)) / (tgamma((float)1/cDist)), 0.5f));
    
    model2 = ((cDist * Yc) / (2*tgamma((float)1/cDist))) * exp(pow(-Yl, cDist) * pow((fabs((float)lc_data2 - MV2)), cDist));
 ...
 */
__kernel void soa_subtract_and_threshold(global int* Data1, global int* Data2,
                                     global float* lcModel1, global float* lcModel2,
                                     global float* M_V1, global float* M_V2, global float* M_V3, global float* M_V4,
                                     __write_only image2d_t output_img,
                                     int width, global int* region_values,
                                     ...
                                     global uchar* output_buffer)
{
    ...
    float model1 = lcModel1[id];
    float model2 = lcModel2[id];
    ...
    float lData = (float) lc_data1;
    float cData = (float) lc_data2;
    float lModel = model1;
    float cModel = model2;
...
__kernel void subtract_and_threshold(global int* Data,
                                     global float* lcModel,
                                     global float* M_V,
                                     __write_only image2d_t output_img,
                                     int width, global int* region_values,
                                     /* For closeAndOpenImage2 - remove after test */
 ...
 */
__kernel void soa_background_update(__read_only image2d_t input_img,
                                global int* Data1, global int* Data2,
                                global float* M_V1, global float* M_V2, global float* M_V3, global float* M_V4)
{
    ...
    MV1 = (Alpha * lData) + (1.0-Alpha) * oldLMean;
    MV3 = Alpha * pow((lData-oldLMean), 2) + (1-Alpha) * MV3;
    
    MV2 = (Alpha * cData) + (1.0-Alpha) * oldCMean;
    MV4 = Alpha * pow((cData-oldCMean), 2) + (1-Alpha) * MV4;
                                    ...
                                    __global float4* M_V)
{
    ...
    MV.s2 = Alpha * pow((lData-oldLMean),2) + (1-Alpha) * MV.s2;
    
    MV.s1 = (Alpha * cData) + (1.0-Alpha) * oldCMean;
    MV.s3 = Alpha * pow((cData-oldCMean),2) + (1-Alpha) * MV.s3;
...
__kernel void erode_image_local(__read_only  image2d_t input_img,
                                __write_only image2d_t output_img,
                                __local uint *l_img /* TODO: allocate this on host instead? */) {
 ...
 */
__kernel void CCL_kernel_1(global uchar* input, global int* output, local uchar* segs, local int* labels, local uint* changed,int width)
{
    int block_width = get_local_size(0);
    int block_height = get_local_size(1);
    int block_width_pad = block_width + 2;
...
__kernel void CCL_kernel_2_testing(global int* labelsInOut, __read_only image2d_t seg_in, local uint* changed, int width, int tileDim)
{
 ...
 */
__kernel void CCL_kernel_2(global int* labelsInOut, read_only image2d_t seg_in, local uint* changed, int width, int tileDim)
{
    /* Sampler for image */
    const sampler_t sampl = CLK_NORMALIZED_COORDS_FALSE |
    CLK_ADDRESS_CLAMP |
...
__kernel void prepareLabelsForScan(global int* outData,
                                   global int* labelsIn,
                                   int width) {
    uint x = (get_group_id(0)*get_local_size(0))+get_local_id(0);
    uint y = (get_group_id(1)*get_local_size(1))+get_local_id(1);
...
__kernel void reindexLabels(global int* labelsOut,
                            global int* labelsIn,
                            global int* scanIn,
                            int width,
                            int numComponents) {
