...
//    float delta_n = delta/n;
    float delta_n = native_divide(delta, n);
//    float delta_n2 = delta_n * delta_n;
    float delta_n2 = native_powr(delta_n, 2);
...
//    MV.s2 = MV.s2 + delta * (lcData.s0 - MV.s0);
    MV.s2 = mad(delta, (lcData.s0 - MV.s0), MV.s2);
...
//    delta_n = delta/n;
    delta_n = native_divide(delta, n);
//    delta_n2 = delta_n * delta_n;
    delta_n2 = pow(delta_n, 2);
    term1 = delta * delta_n * n1;
    // Update mean
    MV.s1 = MV.s1 + delta_n;
    //Update variance
//    MV.s3 = MV.s3 + delta * (lcData.s1 - MV.s1);
    MV.s3 = mad(delta, (lcData.s1 - MV.s1), MV.s3);
    MV.s3 = MV.s3 * first_step; // If first run, set to 0.0
    M4.s1 = M4.s1 + term1 * delta_n2 * (n*n - mad(3, n, 3.0f)) + 6 * delta_n2 * M2.s0 - 4 * delta_n * M3.s1;
    ...
    if(n == N_max) {
        /* Calculate excess kurtosis:
         ...
         (count*M4) / (M2*M2) - 3 */
        float lKurt = native_divide((n*M4.s0), (M2.s0*M2.s0)) - 3;
        float cKurt = native_divide((n*M4.s1), (M2.s1*M2.s1)) - 3;
...
__kernel void background_update(__read_only image2d_t input_img,
    ...
    MV.s2 = mad(Alpha, native_powr((lData-oldLMean), 2), (1-Alpha) * MV.s2);
    
    MV.s1 = (Alpha * cData) + (1.0-Alpha) * oldCMean;
    MV.s3 = mad(Alpha, native_powr((cData-oldCMean), 2), (1-Alpha) * MV.s3);
