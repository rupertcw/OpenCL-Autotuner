...
//        
//        cl_program.build(cl_devices, "-cl-mad-enable");
//        
    ...
    try {
        ...
        for (int i=0;i<N;i++) {
            ...
            cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_convert_rgb_img,
                                             cl::NullRange, cl::NDRange(width, height), // use size
                                             cl::NDRange(local_x, local_y), NULL, &cl_event);
            cl_event.wait();    // remove? for speed
            
            double end = omp_get_wtime();
            cout << "time for convertRGB kernel: " << end-start << endl;
...
//            }
            ...
            cout << "time for kurtosis kernel: " << end2-start2 << endl;
        }
...
void SilhouetteExtraction::updateBgModelForFrame(int workgroupSize)
{
    ...
    try {
        ...
        double end = omp_get_wtime();
        cout << "time for updagteBgModel kernel: " << end-start << endl;
...
void SilhouetteExtraction::subtractBackgroundAndThreshold(int workgroupSize)
{
    ...
    try {
        ...
        double end = omp_get_wtime();
        cout << "time for subAndThresh kernel: " << end-start << endl;
...
void SilhouetteExtraction::labelConnectedComponents(int workgroupSize) {
    ...
    try {
        ...
        cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_ccl_analysis, cl::NullRange, cl::NDRange(width, height), cl::NDRange(local_x, local_y), NULL, &cl_event);
        cl_event.wait();
...
void SilhouetteExtraction::updateBG(int workgroupSize) {
    ...
    cout << "--- SILHOUETTE EXTRACTION --- Update BG" << endl;
    ...
    try {
        ...
        double end = omp_get_wtime();
        cout << "time for updateBG kernel: " << end-start << endl;
    }
    catch (cl::Error cl_err) {
