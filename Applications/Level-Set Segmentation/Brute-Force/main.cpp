...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        cl_cmdqueue = cl::CommandQueue(cl_context, default_device);
        ...
        int tsx, tsy, workgroupSize1, workgroupSize2, bestTx, bestTy, bestWorkgroupSize1, bestWorkgroupSize2;
        ...
        vector<size_t> wis = default_device.getInfo<CL_DEVICE_MAX_WORK_ITEM_SIZES>();
        
//        cout << "Dev Max CU: " << default_device.getInfo<CL_DEVICE_MAX_COMPUTE_UNITS>() << endl;
//        cout << "Dev Max WIS: " << wis[0] << ", " << wis[1] << ", " << wis[2] << endl;
//        cout << "Dev Max WGS: " << default_device.getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>() << endl;
        
        cl_kernel.setArg(3, imageW);
        cl_kernel.setArg(4, imageH);
        
        for (tsy = 3; tsy <= imageW; tsy++)
        {
            cl_kernel.setArg(6, tsy);
            
            for (tsx = 2; tsx <= imageH; tsx++)
            {
                cl_kernel.setArg(5, tsx);
                
                for (workgroupSize1 = 2; workgroupSize1 <= maxWorkgroupSize; workgroupSize1++)
                {
                    int width = imageW;
                    ...
                    for (workgroupSize2 = 1; workgroupSize2 <= maxWorkgroupSize; workgroupSize2++)
                    {
                        ...
                        // RESET DATA
                        cl::Buffer d_phi = cl::Buffer(cl_context, CL_MEM_WRITE_ONLY, sizeof(float)*width*height);
