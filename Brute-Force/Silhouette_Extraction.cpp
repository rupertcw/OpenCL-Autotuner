...
SilhouetteExtraction::SilhouetteExtraction(std::string input, int num_training_frames, bool isFile)
...
int SilhouetteExtraction::getMaxWorkGroupSize(string clFileName){
    ...
    //std::cout << kernels_size << std::endl;
    ...
    try {
        ...
        //std::cout << "Using " << cl_devices.size() << " devices " << std::endl;
        ...
        std::cout << "--- MAIN --- Using platform: "<<default_platform.getInfo<CL_PLATFORM_NAME>()<<"\n";
        ...
        cout << "WG Size1: " << maxWorkgroupSize << endl;
...
void SilhouetteExtraction::initialize(string clFileName) {
  ...
  clFile = oclLoadProgSource(clFileName.c_str(), "", &kernels_size);
  //clFile = oclLoadProgSource("test.cl", "", &kernels_size);
  // clFile = oclLoadProgSource("test.cl", "", &kernels_size);
  if(clFile == NULL) {
    if(debug >= 0) { 
      printf("--- ERROR --- Couldn't open .cl-file %s! Exiting.\n",clFileName.c_str());
  ...
  try {
    ...
    //////////////////////
     ...
     cl::Device default_device = cl_devices[0];
     std::cout<< "--- MAIN --- Using device: " << default_device.getInfo<CL_DEVICE_NAME>() << "\n";
    //////////////////////
...
bool SilhouetteExtraction::performTrainingStep(int workgroupSize) {
  int local_x = workgroupSize;
  int local_y = workgroupSize;
...
void SilhouetteExtraction::convertNextFrame(int workgroupSize) {
  ...
  try {
    ...
    cl_cmdqueue.enqueueNDRangeKernel(cl_kernel_convert_rgb_img,
    				     cl::NullRange, cl::NDRange(width,height), 
    				     cl::NDRange(workgroupSize,workgroupSize), NULL, &cl_event);
